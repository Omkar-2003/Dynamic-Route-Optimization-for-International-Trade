...
function appendDataToCSV(dataAttributes, csvFilePath) {
  ...
  const values = Object.values(dataAttributes)
    .map((value) => {
...
const getRoutes = async (req, res) => {
  try {
    ...
    if (data.routes[0].geometry.coordinates.length <= 10) {
      ...
      console.log("Geometry Coordinates", data.routes[0].geometry.coordinates);
    ...
    const route1_averageConfidence =
      route1_confidences.reduce((acc, curr) => acc + curr, 0) /
      route1_confidences.length;
    ...
    if (data.routes[0].geometry.coordinates.length <= 10) {
      ...
      route1 = {
        ...
        waypoints: trafficCoordinates,
        distance: dataAttributes_OneRoute.route1_distance,
        duration: dataAttributes_OneRoute.route1_duration,
        road_type: dataAttributes_OneRoute.route1_Road_Type,
        traffic: parseFloat(dataAttributes_OneRoute.route1_Traffic),
        road_confidence: dataAttributes_OneRoute.route1_Road_Confidence,
        geopolitical_constraints:
          dataAttributes_OneRoute.route1_Geopolitical_Constraints,
      };
    } else {
      route1 = {
        source: {
        ...
        waypoints: {
          waypoint_1: [Waypoint_1_Lon, Waypoint_1_Lat],
          waypoint_2: [Waypoint_2_Lon, Waypoint_2_Lat],
          waypoint_3: [Waypoint_3_Lon, Waypoint_3_Lat],
          waypoint_4: [Waypoint_4_Lon, Waypoint_4_Lat],
          waypoint_5: [Waypoint_5_Lon, Waypoint_5_Lat],
          waypoint_6: [Waypoint_6_Lon, Waypoint_6_Lat],
          waypoint_7: [Waypoint_7_Lon, Waypoint_7_Lat],
          waypoint_8: [Waypoint_8_Lon, Waypoint_8_Lat],
          waypoint_9: [Waypoint_9_Lon, Waypoint_9_Lat],
        },
        distance: dataAttributes_OneRoute.route1_distance,
        duration: dataAttributes_OneRoute.route1_duration,
        road_type: dataAttributes_OneRoute.route1_Road_Type,
    ...
    if (data.routes.length === 1) {
      if (data.routes[0].geometry.coordinates.length >= 10) {
        return res.status(200).json({
          ...
          optimized_route: route1,
          message: "One Route Exists",
        });
      } else {
        return res.status(200).json({
          available_routes: {
    ...
    const route2_averageConfidence =
      route2_confidences.reduce((acc, curr) => acc + curr, 0) /
      route2_confidences.length;
    ...
    const route2 = {
      ...
      waypoints: {
        waypoint_1: [route_2_Waypoint_1_Lat, route_2_Waypoint_1_Lon],
        waypoint_2: [route_2_Waypoint_2_Lat, route_2_Waypoint_2_Lon],
        waypoint_3: [route_2_Waypoint_3_Lat, route_2_Waypoint_3_Lon],
        waypoint_4: [route_2_Waypoint_4_Lat, route_2_Waypoint_4_Lon],
        waypoint_5: [route_2_Waypoint_5_Lat, route_2_Waypoint_5_Lon],
        waypoint_6: [route_2_Waypoint_6_Lat, route_2_Waypoint_6_Lon],
        waypoint_7: [route_2_Waypoint_7_Lat, route_2_Waypoint_7_Lon],
        waypoint_8: [route_2_Waypoint_8_Lat, route_2_Waypoint_8_Lon],
        waypoint_9: [route_2_Waypoint_9_Lat, route_2_Waypoint_9_Lon],
      },
      distance: data.routes[1].distance,
      duration: data.routes[1].duration,
      road_type: averageImportance2,
      traffic: parseFloat(average2),
      road_confidence: route2_averageConfidence,
      geopolitical_constraints: geopolitical_route2,
    };
    ...
    const { data: optimizedRoute } = await axios.post(
